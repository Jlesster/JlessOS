...
def generate_neovim_theme(material_colors: dict, term_colors: dict, transparent: bool = False,
                         catppuccin_path: str = None, debug: bool = False) -> dict:
    """
    ...
    if catppuccin_path is None:
        catppuccin_path = os.path.expanduser("~/.config/quickshell/ii/scripts/colors/Colors.json")
    ...
    neovim_colors["base"] = "NONE" if transparent else harmonize_hex(cat["base"], accent_argb, BG_HARMONY, BG_THRESH)

    for k in ["mantle", "crust", "surface0", "surface1", "surface2", "overlay0", "overlay1", "overlay2"]:
    ...
    for k in syntax_colors.keys():
        if SYNTAX_TONE_LIFT > 0:
            neovim_colors[k] = argb_to_hex(lift_tone(hex_to_argb(neovim_colors[k]), SYNTAX_TONE_LIFT))
