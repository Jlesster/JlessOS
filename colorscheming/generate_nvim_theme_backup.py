...
def generate_neovim_theme(
    ...
    BG_THRESH = 5.0
    UI_THRESH = 10.0
    SYNTAX_THRESH = 80.0  # Increased from 40.55 to allow more hue rotation
    TEXT_THRESH = 18.0
    ...
    # Syntax accents - MAXIMUM SATURATION for vibrant syntax
    syntax_colors = {
        "rosewater": (SYNTAX_HARMONY, SYNTAX_THRESH, 95),  # Maximized from 90
        "flamingo": (SYNTAX_HARMONY, SYNTAX_THRESH, 95),  # Maximized from 90
        "pink": (SYNTAX_HARMONY, SYNTAX_THRESH, 98),  # Maximized from 92
        "mauve": (SYNTAX_HARMONY, SYNTAX_THRESH, 98),  # Maximized from 95
        "red": (SYNTAX_HARMONY, SYNTAX_THRESH, 95),  # Maximized from 90
        "maroon": (SYNTAX_HARMONY, SYNTAX_THRESH, 92),  # Maximized from 88
        "peach": (SYNTAX_HARMONY, SYNTAX_THRESH, 95),  # Maximized from 90
        "yellow": (
            SYNTAX_HARMONY,
            SYNTAX_THRESH,
            100,
        ),  # MAXIMUM for yellow - override limit
        "green": (SYNTAX_HARMONY, SYNTAX_THRESH, 98),  # Maximized from 95
        "teal": (SYNTAX_HARMONY, SYNTAX_THRESH, 98),  # Maximized from 95
        "sky": (SYNTAX_HARMONY, SYNTAX_THRESH, 98),  # Maximized from 95
        "sapphire": (SYNTAX_HARMONY, SYNTAX_THRESH, 98),  # Maximized from 95
        "blue": (SYNTAX_HARMONY, SYNTAX_THRESH, 98),  # Maximized from 95
        "lavender": (SYNTAX_HARMONY, SYNTAX_THRESH, 98),  # Maximized from 95
    }

for k, (harmony, thresh, target_chroma) in syntax_colors.items():
        raw = harmonize_hex(cat[k], accent_argb, harmony, thresh)
        # FORCE chroma to target value instead of just capping
        raw_argb = hex_to_argb(raw)
        raw_hct = Hct.from_int(raw_argb)
        # Special handling for yellow - force maximum possible chroma
        if k == "yellow":
            target_chroma = min(target_chroma, 100)  # Allow maximum chroma for yellow
            # Force hue towards pure yellow (60 degrees) for maximum vibrancy
            yellow_hue = (raw_hct.hue + 60) / 2  # Pull hue toward true yellow
            neovim_colors[k] = argb_to_hex(Hct.from_hct(yellow_hue, target_chroma, raw_hct.tone).to_int())
        else:
            # Set chroma to exactly the target value for maximum saturation
            neovim_colors[k] = argb_to_hex(Hct.from_hct(raw_hct.hue, target_chroma, raw_hct.tone).to_int())
    ...
    # Force specific tones for optimal saturation and readability
    # Lower tones = more vibrant perception in dark mode
    TONE_MAP = {
        "mauve": 65,  # Lowered for more vibrancy
        "blue": 68,  # Lowered for more vibrancy
        "green": 68,  # Lowered for more vibrancy
        "teal": 68,  # Lowered for more vibrancy
        "yellow": 55,  # Very low tone for maximum yellow vibrancy
        "pink": 68,  # Lowered for more vibrancy
        "sapphire": 68,  # Lowered for more vibrancy
        "red": 65,  # Kept low for intensity
        "peach": 68,  # Lowered for more vibrancy
        "lavender": 68,  # Added for consistency
        "rosewater": 72,  # Higher for readability
        "flamingo": 70,  # Medium for balance
        "sky": 68,  # Lowered for vibrancy
    }
...
def write_neovim_colorscheme(
    ...
    # Generate rainbow delimiter colors with maximum saturation
    def boost_for_rainbow(argb: int, chroma_boost=1.8, min_tone=65) -> str:
        hct = Hct.from_int(argb)
        return argb_to_hex(
            Hct.from_hct(
                hct.hue,
                min(hct.chroma * chroma_boost, 98),  # Increased max chroma
                max(hct.tone, min_tone),  # Lowered min tone for vibrancy
            ).to_int()
        )

    def force_vivid_dark(argb: int, chroma: float, tone: float) -> str:
        hct = Hct.from_int(argb)
        return argb_to_hex(Hct.from_hct(hct.hue, min(chroma, 98), tone).to_int())

    rainbow_colors = {
        "red": boost_for_rainbow(hex_to_argb(neovim_colors["red"]), 1.8, 65),
        "orange": boost_for_rainbow(hex_to_argb(neovim_colors["peach"]), 1.9, 68),
        "yellow": boost_for_rainbow(hex_to_argb(neovim_colors["yellow"]), 2.0, 70),
        "green": boost_for_rainbow(hex_to_argb(neovim_colors["green"]), 1.8, 68),
        "cyan": boost_for_rainbow(hex_to_argb(neovim_colors["teal"]), 1.9, 68),
        "blue": force_vivid_dark(
            hex_to_argb(neovim_colors["sky"]),
            chroma=98,  # Increased from 70
            tone=68,  # Lowered from 74
        ),
        "violet": force_vivid_dark(
            hex_to_argb(neovim_colors["mauve"]),
            chroma=98,  # Increased from 95
            tone=65,  # Lowered from 66
        ),
    }
...
local function setup_highlights()
    ...
    hi("@variable.builtin", {{ fg = colors.pink, style = "italic,bold" }})  # More vibrant
    hi("@variable.parameter", {{ fg = colors.sky, style = "italic" }})  # More vibrant
    hi("@variable.member", {{ fg = colors.teal }})

    hi("@constant", {{ fg = colors.pink, style = "bold" }})  # More vibrant
    hi("@constant.builtin", {{ fg = colors.pink, style = "italic,bold" }})  # More vibrant
    hi("@constant.macro", {{ fg = colors.yellow, style = "bold" }})  # More vibrant

    hi("@module", {{ fg = colors.sapphire, style = "italic" }})
    hi("@label", {{ fg = colors.sapphire }})

    hi("@string", {{ fg = colors.green, style = "bold" }})  # More vibrant
    hi("@string.escape", {{ fg = colors.pink, style = "bold" }})
    hi("@string.regexp", {{ fg = colors.pink, style = "bold" }})
    hi("@character", {{ fg = colors.sky, style = "bold" }})  # More vibrant
    hi("@character.special", {{ fg = colors.pink, style = "bold" }})

    hi("@number", {{ fg = colors.peach, style = "bold" }})  # More vibrant
    hi("@number.float", {{ fg = colors.peach, style = "bold" }})
    hi("@boolean", {{ fg = colors.peach, style = "italic,bold" }})  # More vibrant
    ...
    hi("@operator", {{ fg = colors.sky, style = "bold" }})  # Use theme color instead of hardcoded
    hi("@operator.java", {{ fg = colors.sky, style = "bold" }})

    hi("@keyword", {{ fg = colors.mauve, style = "bold" }})
    hi("@keyword.repeat.java", {{ fg = colors.mauve, style = "italic,bold"}})
    hi("@keyword.conditional", {{ fg = colors.pink, style = "bold,italic" }})  # More vibrant
    hi("@keyword.function", {{ fg = colors.mauve, style = "bold" }})
    hi("@keyword.operator", {{ fg = colors.sky, style = "bold" }})  # More vibrant
    hi("@keyword.return", {{ fg = colors.pink, style = "bold" }})  # More vibrant
    ...
    hi("@lsp.type.parameter", {{ fg = colors.sky, style = "italic,bold" }})  # More vibrant
    hi("@lsp.typemod.variable.readonly", {{ fg = colors.pink, style = "bold" }})  # More vibrant
    hi("@lsp.typemod.variable.declaration", {{ fg = colors.pink, style = "italic,bold" }})  # More vibrant
    hi("@lsp.typemod.variable.static", {{ fg = colors.yellow, style = "bold" }})  # More vibrant
    hi("@lsp.typemod.variable.global", {{ fg = colors.yellow, style = "italic,bold" }})  # More vibrant

    -- Properties and Fields
    hi("@lsp.type.property", {{ fg = colors.teal, style = "bold" }})  # More vibrant
    hi("@lsp.typemod.property.static", {{ fg = colors.sky, style = "italic,bold" }})  # More vibrant
    hi("@lsp.typemod.property.static.java", {{ fg = colors.sky, style = "italic,bold" }})  # More vibrant
    ...
    hi("@lsp.type.enumMember", {{ fg = colors.teal, style = "bold" }})
